...
//设置目标角度-180~180
void set_target_angle(q_pid *angle_pid,float angle)
{
    // 将角度限制在-180到180之间
...
//pid初始化
void angle_controller_init(q_pid *angle_pid)
{
    angle_pid->target = 0.0f;      // 目标设置0
    angle_pid->kp = 0.0f;                // ????
    angle_pid->ki = 0.0f;                // ????
    angle_pid->kd = 0.0f;                // ????
...
//设置PID参数
void set_angle_pid(q_pid *angle_pid,float kp, float ki, float kd,float max_out,float max_i) //设置kp,ki,kd,输出限幅，积分限幅
{
	angle_pid->kp = kp;
	angle_pid->ki = ki;
	angle_pid->kd = kd;
...
//pid实际控制计算，会输出浮点型vw
float angle_controller(q_pid *angle_pid)
{ 
	if(!angle_pid->enable) {
        return 0.0f;
    } //如果未使能，返回0

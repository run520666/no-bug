...
//角度环
extern void set_target_angle(q_pid *angle_pid,float angle); //设置目标角度
extern void set_angle_pid(q_pid *angle_pid,float kp, float ki, float kd,float max_out,float max_i); //设置kp,ki,kd,输出限幅，积分限幅
extern float angle_controller(q_pid *angle_pid); //角度控制
extern void angle_controller_init(q_pid *angle_pid); //角度控制初始化

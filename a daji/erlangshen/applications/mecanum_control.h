...
typedef struct
{
    ...
    uint16_t stp_distance_l; //左激光测距
    uint16_t stp_distance_r; //右激光测距
} mecanum_control_t;
...
extern void set_target_move_to_target(mecanum_control_t *mecanum_control, fp32 x, fp32 y, fp32 angle, fp32 speed, uint8_t act_control,uint8_t stop_control);
extern void move_x(mecanum_control_t *mecanum_control, fp32 angle, fp32 speed);
extern void move_y(mecanum_control_t *mecanum_control, fp32 angle, fp32 speed);
void move_angle(mecanum_control_t *mecanum_control, fp32 angle);
extern void set_target_move_to_target_up(mecanum_control_t *mecanum_control, fp32 x, fp32 y, fp32 angle, fp32 speed,uint8_t act_control,uint8_t stop_control); //上翘翘板用，关了角度环
// 限制轮速在最大值范围内

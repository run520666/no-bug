...
// 获取当前位置到目标点的距离
void mecanum_xy_distance(mecanum_control_t *mecanum_control)
{
    if (mecanum_control == NULL)
    {
        return;
    ...
    for(int i=0; i<4; i++)
    {
        ...
        uint16_t delta_ecd = (ecd >= last_ecd) ? (ecd - last_ecd) : (ecd + ENCODER_RES - last_ecd);
        //计算轮子转动的脉冲数
        mecanum_control->last_ecd[i] = ecd; //更新上次编码器值
        wheel_distance_sum += (delta_ecd / ENCODER_RES) * WHEEL_C; //求单个轮子的移动距离
    }
    float chassis_distance = wheel_distance_sum / 4.0f; //求底盘的平均移动距离

    // 只考虑直线运动，假设朝向角为0
    mecanum_control->current_pos.distance+= chassis_distance;
    // 如果有朝向角，分解到x/y轴
}

...
// 7、8、9号舵机角度参数
int zx20s_7_initialAngle = 55;      // 初始角度
int zx20s_7_moveAngle = 10;         // 转动角度
int zx20s_7_currentAngle = 55;      // 当前角度

int zx20s_8_initialAngle = 0;       // 初始角度
int zx20s_8_moveAngle = 65;         // 转动角度
int zx20s_8_currentAngle = 0;       // 当前角度

int zx20s_9_initialAngle = 160;     // 初始角度
int zx20s_9_moveAngle = 95;         // 转动角度
int zx20s_9_currentAngle = 150;     // 当前角度
...
AngleChannelMap angleMaps[] = {
  {0,    0, 5},     // 0度时，传感器1对应通道0，传感器2对应通道5
  {45,   1, 6},     // 36度时，传感器1对应通道1，传感器2对应通道6
  {90,   2, 7},     // 80度时，传感器1对应通道2，传感器2对应通道7
...
const char* detectColorSensor1() {
  ...
  // 传感器1的暗光补偿参数
  int r = constrain(rgb[0] + 70, 0, 255);
  int g = constrain(rgb[1] + 80, 0, 255);
  int b = constrain(rgb[2] + 60, 0, 255);
  ...
  // 2. 判断黄色（传感器1的阈值）
   else if (hsl[0] >= 150 && hsl[0] <= 210) {
    if (hsl[1] > 10 && hsl[2] > 90 &&
        r > 160 && g > 160 &&
        abs(r - g) <10 && abs(r-b) < 5 && abs(g - b) < 10 ) {
      return "Yellow";
    }
  }
  
  // 3. 判断绿色（传感器1的阈值）
  else if (hsl[0] >= 120 && hsl[0] <= 140) {
    if (hsl[1] > 60 && hsl[2] > 60 &&
        g > r + 20 && g > 160) {
      return "Green";
    }
  }
...
const char* detectColorSensor2() {
  ...
  // 2. 判断黄色（传感器2的阈值）
  else if (hsl[0] >= 190 && hsl[0] <= 220) {
    if (hsl[1] > 25 && hsl[2] > 60 &&
        r > 140 && g > 130 &&
        abs(r - g) < 15 ) {
      return "Yellow";
    }
  ...
  // 3. 判断绿色（传感器2的阈值）
  else if (hsl[0] >= 125 && hsl[0] <= 170) {
    if (hsl[1] > 20 && hsl[2] > 30 && hsl[1] > hsl[2]
       && b > r && g > r + 10 &&g > 120) {
      return "Green";
    }
  }
